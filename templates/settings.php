...
$current_tab = isset ( $_GET['tab'] ) ? sanitize_text_field( $_GET['tab'] ) : $tabs_slugs[0];
...
<div class="notice notice-warning">
	<p>由于涉及小程序的改动，暂不提供后台直接更新。如提示插件更新，请到<a href="https://saledash.cn/" target="_blank">官网</a>，点击<code>定价 - 试用版</code>下载最新版，然后一并更新插件和小程序</p>
</div>

<nav class="nav-tab-wrapper">
<?php foreach( $tabs as $key => $label ): ?>
	<a href="#tab-<?php echo esc_attr( $key ) ?>" class="nav-tab <?php if( $key == $current_tab ) echo 'nav-tab-active' ?>" data-name="<?php echo esc_attr( $key ) ?>"><?php echo esc_attr( $label ) ?></a>
<?php endforeach; ?>
<a href="https://saledash.cn/" class="nav-tab" target="_blank">升级专业版<span class="badge" style="background-color:#F44336;border-radius:50%;"></span></a>
</nav>
...
<form id="setting-form" method="post" action="options.php" enctype="multipart/form-data">
	...
	<div id="tab-<?php echo esc_attr( $key ) ?>" <?php if( $current_tab == $key ) echo 'class="tab-content-active"' ?>>
		...
		<tr><td colspan="2" style="padding: 0;">
			...
			<?php if( ! empty( $settings['banner'] ) ): ?>
				...
				<div class="banner">
					...
					<button><div class="thumb" style="background-image:url(<?php echo esc_attr( $banner['image'] ) ?>)"></div></button>
					<input type="hidden" name="w2w-settings[banner][<?php echo $i ?>][image]" value="<?php echo esc_attr( $banner['image'] ) ?>"/>
					<input type="text" name="w2w-settings[banner][<?php echo $i ?>][url]" value="<?php echo esc_attr( $banner['url'] ) ?>" placeholder="链接"/>
				</div>
				<?php endforeach; ?>
			<?php else: ?>
				<div class="banner">
	...
	?>
		...
		<?php foreach( $pages as $page ): ?>
			<option value="<?php echo esc_attr( $page->ID ) ?>" <?php selected( isset( $settings['about_page'] ) && $settings['about_page'] == $page->ID ) ?>>
		...
		<input type="number" id="w2w-settings[related_products_quantity]" name="w2w-settings[related_products_quantity]" value="<?php echo isset( $settings['related_products_quantity'] ) ? esc_attr( $settings['related_products_quantity'] ) : '' ?>" placeholder="4" />
	...
	<?php submit_button(); ?>
	<input type="hidden" name="w2w-settings[current_tab]" value="<?php echo esc_attr( $current_tab ) ?>">
</form>
</div>
<script>
;(function($) {
