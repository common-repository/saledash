<?php
/**
 * Plugin Name: SaleDash - WC微信商城小程序
 * Plugin URI: https://saledash.cn/
	...
	class SaleDash_Mini {
		...
		public function login_with_session() {
			...
			if( ! empty( $_REQUEST['w2w_session'] ) ) {
				SaleDash_Util::login( wc_clean( $_REQUEST['w2w_session'] ) );
			}
		}
		...
		public function send_need_login_header( $result, $server, $request ) {
			...
			if( ! empty( $request['w2w_session'] ) ) {
				...
				$session = SaleDash_Util::get_session( $request['w2w_session'] );
		...
		?>
		<style>
		#adminmenu .toplevel_page_w2w-settings img {
			width: 20px;
		...
		public function add_action_links( $links ) {
			...
			$setting_link = array(
				...
				'<a href="https://saledash.cn/" style="color:#f00;font-weight:bold;" target="_blank">升级专业版！</a>',
			);
		...
		<input type="text"
				...
				value="<?php echo isset( $settings['order_number_prefix'] ) ? esc_attr( $settings['order_number_prefix'] ) : '' ?>"
		/>
		...
		public function plugin_setup_menu() {
			...
			$menu_title = 'SaleDash';
			$is_setting_page = $pagenow == 'admin.php' && isset( $_GET['page'] ) && preg_match( '/^w2w/', $_GET['page'] );
			$is_setting_page = apply_filters( 'w2w_is_setting_page', $is_setting_page );
			if( $is_setting_page ) {
				$icon = WP_W2W_URL . 'assets/images/saledash-white.svg';
			} else {
				$icon = WP_W2W_URL . 'assets/images/saledash.svg';
			}
			
			$submenu['options-general.php'][] = array( $menu_title . '设置', 'manage_woocommerce', admin_url( "admin.php?page=w2w-settings" ) );
			add_menu_page( $menu_title, $menu_title . '小程序', 'manage_woocommerce', 'w2w-settings', array( $this, 'settings_init' ), $icon, 56 );
			add_submenu_page( 'w2w-settings', $menu_title . '设置', '基本设置', 'manage_woocommerce', 'w2w-settings' );
		...
		public function pre_update_option( $value, $old_value, $option ) {
			...
			if( isset( $value['current_tab'] ) ) {
				$_REQUEST['_wp_http_referer'] = sanitize_url( add_query_arg( 'tab', $value['current_tab'], $_REQUEST['_wp_http_referer'] ) );
