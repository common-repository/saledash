...
class W2W_Template_Message {
		...
		<tr><td style="padding: 0;">
			...
			<table class="widefat fixed my-templates">
				...
				<tr>
					...
					<td><?php $content = preg_replace('/\{\{.*?\}\}/', ', ', $template['content'] ); echo esc_html( trim( trim( $content ), ', ' ) ); ?></td>
					<td><?php echo esc_html( $template['template_id'] ) ?></td>
					<td>
					<?php
						foreach( $this->default_templates as $name => $args ) {
							if( isset( $settings['templates'][$name] ) && $settings['templates'][$name] == $template['template_id'] ) {
								echo esc_html( $this->template_labels[$name] ) . '<i class="dashicons dashicons-yes"></i>';
								break;
							}
						}
					?>
	...
	<?php foreach( $this->default_templates as $name => $args ): ?>
		<input type="hidden" name="w2w-settings[templates][<?php echo $name ?>]" value="<?php echo isset( $settings['templates'][$name] ) ? esc_attr( $settings['templates'][$name] ) : '' ?>" />
	<?php endforeach; ?>
	...
	public function save_order_formid( $order_id, $posted_data, $order ) {
		if( isset( $_REQUEST['form_id'] ) ) {
			update_post_meta( $order_id, 'w2w_form_id', wc_clean( $_REQUEST['form_id'] ) );
		}
	}
