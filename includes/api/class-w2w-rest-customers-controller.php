...
class W2W_REST_Customers_Controller extends WC_REST_Controller {
	...
	public function login( $request ) {
		...
		// 解密用户信息
		$err_code = SaleDash_Util::decrypt_data( $settings['appid'], $api_result['session_key'], $request['encryptedData'], urldecode( $request['iv'] ), $data );
		...
		//更新顾客信息
			...
			if( isset( $data['unionId'] ) ) {
				...
				update_user_meta( $customer->ID, 'w2w_unionid', $data['unionId'] );
			}
		}
		
		// 生成SESSION
		$w2w_session = SaleDash_Util::generate_session( $customer->ID );
		SaleDash_Util::login( $w2w_session );
